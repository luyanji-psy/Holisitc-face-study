...
[pressTime, keyCode, dsecs] = KbWait([],0); 
...
dataHeaders = {'Experiment', 'Participant','Trial','SubTrial','Condition','CuedHalf','Congruency', 'Alignment', 'SameDifferent',...
    'FaceGroup','StudyUpper','StudyLower','TargetUpper','TargetLower',...
    'thisResponse','isCorrect','reactionTime', 'studyDuration', 'targetDuration', 'maskDuration', 'trialEndTime'};
trialData =   { '109_cue', participantNum,ttn,NaN,'Complete_cue', cuedHalf(2-ed(ttn).topIsCued), congruency(2-ed(ttn).isCongruent), alignment(2-ed(ttn).isAligned), sameDifferent(2-ed(ttn).cuedIsSame ) ,...
    faces(thisFaceSet(1),ed(ttn).faceGroup).group, faces(thisFaceSet(1),ed(ttn).faceGroup).filename, faces(thisFaceSet(2),ed(ttn).faceGroup).filename, faces(thisFaceSet(3),ed(ttn).faceGroup).filename, faces(thisFaceSet(4),ed(ttn).faceGroup).filename,...
    Resp,  ACC, reactionTime, studyDuration, 'infinite', maskDuration, datestr(now,'yyyy-mm-dd-HH:MM:SS')};
