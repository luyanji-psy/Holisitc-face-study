...
`Con_Ali`, `Cue_Con_Sam`, `Con_Ali_Sam`, and `Cue_Con_Ali_Sam` were removed from extended model (`glmm_resp_etd`) due to that the variances they explained were smaller than 0.1%, making `glmm_rt_rdc`.
...
```{r rePCA for glmm_rt_etd}
summary(rePCA(glmm_rt_etd))
```

`Con_C`, `Ali_Sam`, `Cue_Con`, `Ali_C`, and `Cue_Sam` were removed from extended model (`glmm_resp_etd`) due to that the variances they explained were smaller than 1%, making `glmm_resp_etd1`.

```{r rt exteded model glmm_rt_etd1}
...
if (!file.exists(file_rt_etd1)) {
  ...
  # save(glmm_rt_etd1, file = glmm_rt_etd1)
} else {
  load(file_rt_etd1)
}
...
`Cue_Ali`, `Sam_C`, and `Con_Sam` were removed from extended model (`glmm_resp_etd1`) due to that the variances they explained were smaller than 1%, making `glmm_resp_etd2`.
...
if (!file.exists(file_rt_etd2)) {
  ...
  # save(glmm_rt_etd2, file = glmm_rt_etd2)
} else {
  load(file_rt_etd2)
}
...
```{r rePCA for glmm_rt_etd2}
summary(rePCA(glmm_rt_etd2))
```


`Cue_Ali_Sam` was removed from extended model (`glmm_resp_etd2`) due to that the variances it explained were smaller than 1%, making `glmm_resp_etd3`.

```{r rt exteded model glmm_rt_etd3}
file_rt_etd3 <- file.path(folder_lmm, "rt_lmm_etd3.RData")

# fit the etd3 model
if (!file.exists(file_rt_etd3)) {
  glmm_rt_etd3 <- lmer(
    log(RT) ~ Cue * Congruency * Alignment * SameDifferent + Probability + 
      (Cue_C +  # Con_C + Ali_C + Sam_C +
          # Cue_Ali + Cue_Con + Con_Ali + Ali_Sam + Cue_Sam + Con_Sam +
  ...
  # save(glmm_rt_etd3, file = glmm_rt_etd3)
} else {
  load(file_rt_etd3)
}

print(summary(glmm_rt_etd3), corr = FALSE)
```

```{r rePCA for glmm_rt_etd3}
summary(rePCA(glmm_rt_etd3))
```

`Cue_Ali_Sam` was removed from extended model (`glmm_resp_etd3`) due to that the variances it explained were smaller than 1%, making `glmm_resp_etd4`.

```{r rt exteded model glmm_rt_etd4}
file_rt_etd4 <- file.path(folder_lmm, "rt_lmm_etd4.RData")

# fit the etd4 model
if (!file.exists(file_rt_etd4)) {
  glmm_rt_etd4 <- lmer(
    log(RT) ~ Cue * Congruency * Alignment * SameDifferent + Probability + 
      (Cue_C   # Con_C + Ali_C + Sam_C +
          # Cue_Ali + Cue_Con + Con_Ali + Ali_Sam + Cue_Sam + Con_Sam +
  ...
  # save(glmm_rt_etd4, file = glmm_rt_etd4)
} else {
  load(file_rt_etd4)
}

print(summary(glmm_rt_etd4), corr = FALSE)
```
...
# compare the extended and reduced model
anova(glmm_rt_etd4, glmm_rt_rdc, refit = FALSE)
```
...
```{r emm rt}
emm_options(lmerTest.limit = 1e6)
emm_options(pbkrtest.limit = 1e6)

test1 <- ref_grid(glmm_rt_opt)
pigs.remm.s <- emmeans(regrid(pigs.rg), "source")

emm_rt <- emmeans(glmm_rt_opt, ~ Congruency + Alignment)
