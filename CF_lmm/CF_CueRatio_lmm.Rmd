...
#   glmm_rt_max <- glmer(
#     Resp ~ Cue * Congruency * Alignment + Probability +
#       (Cue * Congruency * Alignment | Participant), # Con_Ali_Sam
#     family = lognormal(),
#     data = df_lmm,
#     control = glmerControl(optimizer = "optimx", # calc.derivs = FALSE,
#                            optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE))
...
Though the first 7 random effects explained more than 99.9% total variances, no warning of failing to converge was observed. Thus, we created the extended model based on the zcp model. In other words, the extend model is the same as the maximal model.
...
`Con_C`, `Con_Ali`, `Cue_Con`, and `Ali_C` were removed from extended model (`glmm_resp_etd`) due to that the variances they explained were smaller than 1%, making `glmm_resp_etd1`.
...
`Cue_Con_Ali` was removed from extended model (`glmm_resp_etd1`) due to that the variances they explained were smaller than 1%, making `glmm_resp_etd2`.
...
# compare the extended and reduced model
anova(glmm_rt_etd2, glmm_rt_zcp, refit = FALSE)
```
...
```{r emm rt}
emm_options(lmerTest.limit = 62000)
emm_options(pbkrtest.limit = 62000)
...
```{r}
emmip(regrid(emm_rt), Congruency ~ Alignment | Cue )
```
