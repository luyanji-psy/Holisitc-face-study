...
`Con_C` was removed from extended1 model.
...
```{r resp exteded9 model}
file_resp_etd9 <- file.path(folder_lmm, "Resp_lmm_etd9.RData")

# fit the etd9 model
if (!file.exists(file_resp_etd9)) {
  ...
  # save(glmm_resp_etd9, file = glmm_resp_etd9)
} else {
  load(file_resp_etd9)
}

print(summary(glmm_resp_etd9), corr = FALSE)
```

`Cue_Con_Ali_Sam` was removed.

```{r resp exteded10 model}
file_resp_etd10 <- file.path(folder_lmm, "Resp_lmm_etd10.RData")

# fit the etd10 model
if (!file.exists(file_resp_etd10)) {
  ...
  # save(glmm_resp_etd10, file = glmm_resp_etd10)
} else {
  load(file_resp_etd10)
}

print(summary(glmm_resp_etd10), corr = FALSE)
```
...
anova(glmm_resp_etd10, glmm_resp_rdc, refit = FALSE)
```
...
emm_cf <- contrast(emm_resp, interaction = "pairwise", by = "Cue")
...
emm_fi <- contrast(emm_resp, interaction = "pairwise", by = c("Cue", "Congruency"), infer=TRUE)
