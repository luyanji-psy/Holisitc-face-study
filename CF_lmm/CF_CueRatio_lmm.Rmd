...
```{r fi E1 d, fig.width=3, fig.asp=.65}
...
```{r fi E1 rt, fig.width=3, fig.asp=.65}
...
```{r plot for publication E2 d slides, fig.width=3, fig.asp=.6}
plot_E2_cf_d <- summary(emm_E2_d[1:16], infer = c(TRUE, FALSE)) %>% 
  ...
  facet_grid(Probability ~Cue, switch = "x",
             labeller = labeller(Probability = c(`0.25` = "25%", `0.75` = "75%"))) +
  coord_cartesian(ylim = ylimit_cf_d) +  # set the limit for y axis c(0, 1100)
  labs(x = "Cues", y = expression("Sensitivity"~italic("d'")), fill = "Congruency") +  # set the names for main, x and y axises
...
```{r fi E2 d, fig.width=3, fig.asp=.65}
...
```{r plot for publication E2 rt slides, fig.width=3, fig.asp=.6}
...
```{r fi E2 rt, fig.width=3, fig.asp=.65}
...
```{r plot for publication E12 rt slides, fig.width=3, fig.asp=.6}
