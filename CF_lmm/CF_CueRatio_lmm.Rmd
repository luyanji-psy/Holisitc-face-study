...
```{r combined plot d E1, fig.width=6, fig.asp=.5}
...
```{r}
summary(contrast(emm_E1_d_fi, method = list("faci-inte"=c(1, 1)), by = "Cue"), infer = TRUE)
```

```{r}
# summary(contrast(emm_E1_d, method = list("faci-inte"=c(1, -1, 1, -1 )), by = "Cue"), infer = TRUE)
```


```{r}
# emmeans(glmm_E1_resp_opt, ~ Alignment + SameDifferent | Cue ) %>% 
...
```{r combined plot rt E1, fig.width=6, fig.asp=.5}
...
```{r}
summary(contrast(emm_E1_rt_fi, method = list("faci-inte"=c(1, 1)), by = "Cue"), infer = TRUE)
```

```{r}
emmeans(regrid(emm_E1_rt), ~ Alignment | Cue) %>% 
  # regrid() %>% 
  contrast(interaction = "pairwise") %>% 
  summary(infer = TRUE)
```
...
```{r combined plot d E2, fig.width=6, fig.asp=.5}
...
```{r}
summary(contrast(emm_E2_d_fi, method = list("faci-inte"=c(1, 1)), by = c("Cue", "Probability")), infer = TRUE)
```

```{r}
# contrast(emmeans(emm_E2_resp, ~ Alignment + SameDifferent | Cue + Probability),
...
```{r combined plot rt E2, fig.width=6, fig.asp=.5}
...
```{r combined plot d E12, fig.width=6, fig.asp=.5}
...
```{r combined plot rt E12, fig.width=6, fig.asp=.5}
